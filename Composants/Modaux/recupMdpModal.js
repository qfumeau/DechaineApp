...
export default class RecupMdpModal extends React.Component {
  ...
  recupMdp() {
    if (adresseMail) {
      firebase
        .auth()
        .sendPasswordResetEmail(adresseMail)
        .then(() => alert('Mail envoyé !'))
        .catch(error => {
          if (
            error.toString() == 'Error: The email address is badly formatted.'
          ) {
      ...
      Alert.alert('Champ vide', 'Veuillez saisir une adresse mail');
    }
  }
        ...
        >
          ...
          >
            ...
            <TextInput
              ...
              autoCorrect={false}
              onChangeText={text => {
                this.setState({ adresseMailState: text }), (adresseMail = text);
              }}
            ...
            <Image
              source={require('../../img/security3.png')}
              style={{ marginTop: 45, width: '30%', height: '15%' }}
            />
          </ImageBackground>
        </Modal>
        <TouchableHighlight
